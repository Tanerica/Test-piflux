...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser('Choose path to compute score')
    parser.add_argument('--normal_dir', type=str, default="/workspace/normal/normal_output", help='Path to normal schnell')
    parser.add_argument('--expected_dir', type=str, default="/workspace/xDiT/results", help='Path to optimized schnell')
    args = parser.parse_args()
    img_dir1 = args.normal_dir
    img_dir2  = args.expected_dir
    
    num_eval_images = 100
    final_scores = []
    for i in range(num_eval_images):
        img1_path = os.path.join(img_dir1, f'{i}.png')
        img2_path = os.path.join(img_dir2, f'{i}.png')
    ...
    print(f'FINAL SCORES:--{args.expected_dir}', final_scores)
